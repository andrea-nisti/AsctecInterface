...
void velocityCommandCallback(const geometry_msgs::Point pose_msg);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber pos_sp_sub = n.subscribe("/firefly/position_control/vel_sp",1,&velocityCommandCallback);
    ros::Subscriber vel_sp_sub = n.subscribe("/firefly/position_control/pos_sp",1,&positionCommandCallback);

    chatter_pub = n.advertise<mav_msgs::RollPitchYawrateThrust>("/firefly/command/roll_pitch_yawrate_thrust", 1);
    vel_err_pub = n.advertise<geometry_msgs::Point>("/bocheccazzomefrega",1);
...
void velocityCommandCallback(const geometry_msgs::Point vel_msg){
